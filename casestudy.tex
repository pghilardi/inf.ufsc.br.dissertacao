...
\begin{landscape}
  \begin{figure*}[tb]
	...
	Room composed with Log.}\label{fig:case_study_compound_1}
  \end{figure*}
\end{landscape}
...
The second case study\ldots

\section{Discussão}

Applying the proposed approach in a case study shows how it facilitates the
understanding of aspect-oriented applications, by enabling the toggling of
views, viewing only the core concerns or a composition between the core and
crosscutting concerns, without effort from the developer. The composed models
shows the effect of aspects in a system, with each aspect differentiated by a unique
color that represents it. The SEA/Aspect allows the separation of concerns in
the early development phases, which is one of the objectives of the AOM
approaches too.