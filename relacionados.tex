...
no trabalho proposto por Wimmer \cite{wimmer:2011:SUA:1978802.1978807} e outros são propostos por este trabalho. Para permitir uma melhor
visualização das tabelas de comparação, as abordagens são abreviadas com as três primeiras letras do autor principal. A seguir, são apresentados os
critérios baseados no trabalho de Wimmer \cite{wimmer:2011:SUA:1978802.1978807} e alguns critérios propostos por esta dissertação. Para cada
critério, apresenta-se a tabela de comparação entre as abordagens e uma pequena discussão sobre os resultados de cada tabela.

\subsection{Critérios de Linguagem}
...
A tabela \ref{tab:comparison_table_language} apresenta a comparação das abordagens em relação a critérios de linguagem. 
...
\begin{table*}[h]
	...
	\begin{tabular}{ | c | c | c | c | c | c | c | c | c | c | }
		...
		\hline
		 L.E & MM & Perfil & Perfil & MM & Perfil & MM & Perfil & MM & Perfil \\
		\hline
	...
	\begin{tabular}{  p{16.81cm}  }
		Legenda: \\
	\end{tabular}

	\begin{tabular}{ | p{16.81cm} | }
		\hline
		MM (Meta-Modelo), Perfil (Perfil UML), DC (Diagrama de Classes), DComp (Diagrama de Componentes), DEC (Diagrama de Estrutura Composta), DI
...
Observa-se que a maior parte das abordagens especificam aspectos com a segunda versão da UML. As abordagens limitadas a primeira versão da UML não
contemplam todas as construções da linguagem e, não são suportadas por qualquer ferramenta CASE. Em relação a forma de extensão, algumas
abordagens estendem por um perfil UML, enquanto outras estendem diretamente o meta-modelo. A extensão por meta-modelo não permite utilizar a abordagem em outra
ferramenta diferente da que foi implementada originalmente, diminuindo o potencial de reuso. Um perfil UML pode ser importado em ferramentas CASE que
suportem a importação de perfis. A maior parte das abordagens utiliza diagramas de classe para modelagem estrutural. Em relação ao comportamento, a
maior parte das abordagens modela com diagramas de sequência.

\subsection{Critérios de Modelos}
...
\begin{itemize}
	...
	\item \textbf{Tratamento de Conflitos (T.C)}: Abordagens podem evitar conflitos, detectar e resolver automaticamente, detectar e resolver
	manualmente, ou, não tratar conflitos.
	...
	sistemas orientados a aspectos em alto ou baixo nível de abstração. É possível que uma abordagem representa os conceitos nos dois níveis.
\end{itemize}

\subsection{Critérios de Ponto de Junção e Aviso}
...
\begin{itemize}
	...
	\item \textbf{Ponto de Corte (P.C)}: Avalia se a representação de pontos de corte é realizada dentro dos padrões de especificação de pontos de corte
	(AspectJ, OCL, etc) ou é realizado através de alguma linguagem proprietária. Também existe a possibilidade de pontos de corte não serem representados.
	\item \textbf{Representação de Ponto de Corte (R.P.C)}: Distingue a forma de representação de pontos de corte, a qual pode ser textual ou gráfica,
	com algum diagrama da linguagem. 
	\item \textbf{Composição de Ponto de Corte (C.P.C)}: Avalia se a abordagem permite realizar a composição de pontos de corte, com os operadores
	lógicos AND e OR. 
	\item \textbf{Aviso (A)}: Avalia se a representação de avisos é realizada. Se for realizada, discrimina quais tipos de aviso são suportados: antes,
	durante e depois.
\end{itemize}

\subsection{Critérios de Composição}
...
\begin{itemize}
	...
	\item \textbf{Método de Comparação (M.C)}: Este critério avalia o método de comparação utilizado no algoritmo para composição de modelos. A
	comparação pode ser realizada por nome, por assinatura ou não realizada.
...
\subsection{Critérios Propostos}
...
\subsubsection{Critérios de Especificação de Aspectos}
...
\begin{itemize}
	...
	\item \textbf{Captura de Múltiplos Pontos de Junção (C.M.P.C)}: Distingue quais abordagens
	permitem a captura de múltiplos pontos de junção de maneira praticável, com o uso de expressões regulares (\textit{wildcards}).
\end{itemize}

\subsubsection{Critérios de Visualização}
